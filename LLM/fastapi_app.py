...
@app.get("/debug/runtime-fingerprint")
def runtime_fingerprint():
    """
    Return a lightweight runtime fingerprint to verify which code is actually
    running on a live instance.
    """
    try:
        chain_file = inspect.getsourcefile(generate_feeding_program_meal_plan) or "unknown"
        validate_src = inspect.getsource(generate_feeding_program_meal_plan)
        validate_hash = hashlib.sha256(validate_src.encode("utf-8")).hexdigest()[:16]

        file_mtime = None
        if chain_file != "unknown" and os.path.exists(chain_file):
            file_mtime = datetime.fromtimestamp(os.path.getmtime(chain_file)).isoformat()

        return {
            "status": "ok",
            "service": "nutrition-llm-api",
            "api_file": __file__,
            "feeding_program_chain_file": chain_file,
            "feeding_program_chain_mtime": file_mtime,
            "function_fingerprint": validate_hash,
            "app_env": os.getenv("APP_ENV", ""),
            "seeds_enabled": os.getenv("FEEDING_PROGRAM_SEEDS_ENABLED", "unset"),
